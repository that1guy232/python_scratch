...
class TestCPU_LDA(unittest.TestCase):

    def setUp(self):
        """Initialize the CPU for each test."""
        self.cpu = CPU(Memory())

    def test_load_A_positive_value(self):
        self.cpu.reset()
        self.cpu.memory.load_data(0x0000, [OPCODES['LDA']['code'], 0x7F])  # Load immediate value 0x7F
    ...
    def test_load_A_negative_value(self):
        self.cpu.reset()
        self.cpu.memory.load_data(0x0000, [OPCODES['LDA']['code'], 0x80])  # Load immediate value 0x80
